...
module.exports = function supercall(babel) {
  ...
  function testMemberExpressionWithIdentifiers(node, name) {
    ...
    return (
      ...
      node.object.name === pair[0] &&
      t.isIdentifier(node.property) &&
      node.property.name === pair[1]
    );
  }
  ...
  return {
    visitor: {
      CallExpression(path) {
        ...
        const { node } = path;
        if (
          testMemberExpressionWithIdentifiers(possibleConstructorReturn, 'babelHelpers.possibleConstructorReturn') &&
          t.isMemberExpression(node.callee) &&
          t.isIdentifier(node.callee.property) &&
          /^(call|apply)$/.test(node.callee.property.name) &&
          ...
          if (testMemberExpressionWithIdentifiers(requestForPrototype, 'Object.getPrototypeOf')) {
            ...
            const checkForNativeSuperCallAvailability = t.binaryExpression(
              '!==',
              t.unaryExpression('typeof', t.identifier('Reflect')),
              t.stringLiteral('undefined')
            );
            const conditionalNativeSuperCall = t.expressionStatement(
              t.conditionalExpression(checkForNativeSuperCallAvailability, nativeSuperCall, node)
            );
